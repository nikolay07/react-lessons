...
export const onListClick = (event) => {
  const check = event.target.classList.contains(
    "list-item__checkbox"
  );
  const deleteTaskItem = event.target.classList.contains(
...
export const onToggleTask = (event) => {
  const isCheckbox = event.target.classList.contains(
    "list-item__checkbox"
  );
  if (!isCheckbox) return;
  const taskId = event.target.dataset.id;
  const tasksList = getItem("tasksList");
  const { text, createDate } = tasksList.find(
    (task) => task.id === taskId
  );
